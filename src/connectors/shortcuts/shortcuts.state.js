...
function* selectItem(next) {
  ...
  const itemsIds = yield select(getPageSavedIds, section)
  const total = itemsIds?.length
  const currentPosition = itemsIds.indexOf(currentId)

  if (next) {
    const nextPosition = currentPosition < 0 ? 0 : currentPosition + 1
    const nextId = itemsIds[nextPosition] ? itemsIds[nextPosition] : false
    return yield put({
  ...
  const prevId = itemsIds[prevPosition] ? itemsIds[prevPosition] : false
  return yield put({
    type: SHORTCUT_SET_CURRENT_ITEM,
    currentId: prevId,
