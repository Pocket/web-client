...
export function SearchFilterMenu({ filter, total, query }) {
  ...
  const searchQuery = query ? `?q=${query}&st=saves` : ''
    ...
    <div className={cx(filterStyle, 'filter-wrapper')}>
      ...
      {shown ? (
        <div className="filter-menu" ref={popBody}>
          <div>
            <Link href={`/search${searchQuery}`} legacyBehavior>
          ...
          <div>
            <Link href={`/search${searchQuery}&ft=unread`} legacyBehavior>
          ...
          <div>
            <Link href={`/search${searchQuery}&ft=archive`} legacyBehavior>
